...
df1 = pd.read_excel('dummyone.xlsx', 'Controllers', na_values=['NA'])
df2 = pd.read_excel('dummysecond.xlsx', 'Controllers', na_values=['NA'])

# Make sure we order by account number so the comparisons work
df1.sort(columns="Model")
df1=df1.reindex()
df2.sort(columns="Model")
df2=df2.reindex()
...
diff_output[(diff_output.has_change == 'Y')].to_excel('diff.xlsx',index=False,columns=["Name","Model","IP Address"])
